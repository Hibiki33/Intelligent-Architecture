...
class DATA_OBJECT(object):
    def __init__(self,
        ...
        if one_hot:
            # If the one_hot labeling is desired, number of classes must be defined as one of the arguments of
            # DATA_OBJECT class!
...
def provide_data(mnist):
    ...
    # The ?_images(? can be train, validation or test) must have the format of [num_samples, rows, columns,
    # depth] after extraction from data. The ?_labels(? can be train, validation or test) must have the format of [
    # num_samples,] after extraction from data.
